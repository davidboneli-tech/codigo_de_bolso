...
**380 fichas · 28 categorias · Versão 0.6.3**
...
A coleção inclui 815 exemplos, código agrupado por tarefa, 22 imagens de gráficos e 75 ilustrações 3D para consulta offline. As ilustrações aparecem em fichas selecionadas, com legendas próprias; comandos nas explicações ficam em negrito. As áreas possuem sublistas para manter a navegação compacta. Power BI permanece como possibilidade de expansão.
...
**Versão documentada: 0.6.3.** O manifesto define Android 8.0 como versão mínima. A compatibilidade precisa ser verificada no aparelho; não há declaração de testes em todos os modelos Android.

A versão 0.6.3 acrescenta 20 ilustrações, totalizando 75 fichas ilustradas, com 1.270 verificações de interface em Chromium e assinatura v2/v3 do APK verificada. A migração da 0.6.2 preserva favoritos e preferências. David confirmou a instalação e encontrou as imagens na 0.6.2; a instalação da 0.6.3 no aparelho ainda precisa ser conferida.

A revisão anterior 0.6.1 simplificou as explicações e reuniu linhas de uma mesma tarefa. Foram executados 8 exemplos novos e 1.109 verificações de interface; a lógica e as saídas dos outros 807 exemplos foram preservadas. O APK foi compilado e teve a assinatura verificada. David confirmou o funcionamento da 0.6.0 no aparelho; a instalação da 0.6.1 ainda precisa ser conferida. Veja [escopo e limites dos testes](docs/escopo_e_validacao.md).
